...
import com.fundoonotes.messaging.RabbitReminderProducer;
...
public class NoteServiceImpl implements NoteService {
	...
	public NoteServiceImpl(NoteRepository noteRepository, UserRepository userRepository, TagRepository tagRepository,
			NoteMapper noteMapper, ReminderProducer reminderProducer, RabbitReminderProducer rabbitReminderProducer) {
	...
	private Note getOwnedNote(int noteId, int userId) {
		...
		return noteRepository.findByNoteIdAndOwner(noteId, owner).orElseThrow(() -> new NoteNotFoundException(noteId));
	}
	...
	public NoteResponseDTO setReminder(int noteId, int userId, LocalDateTime reminderAt) {
		...
		if (reminderAt.isBefore(LocalDateTime.now())) {
			throw new InvalidNoteStateException("Reminder time must be in the future");
		}
		note.setReminderAt(reminderAt);

		Note savedNote = noteRepository.save(note);

		reminderProducer.sendNoteReminderMessage(userId, savedNote.getNoteId(), savedNote.getTitle());

		return noteMapper.toResponseDTO(savedNote);
	}

	@Override
	public NoteResponseDTO sendRabbitReminder(int noteId, int userId) {

		Note note = getOwnedNote(noteId, userId);

		rabbitReminderProducer.sendReminder(note.getTitle(), userId);

		return noteMapper.toResponseDTO(note);
	}

	@Override
	public List<Note> getActiveNoteEntities(int userId) {

		User owner = getUser(userId);

		return noteRepository.findByOwnerAndState(owner, NoteState.ACTIVE);
	}
}